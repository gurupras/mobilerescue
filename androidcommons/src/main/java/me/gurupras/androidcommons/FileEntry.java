...
public class FileEntry extends LinkedList<FileEntry> {
	...
	public FileEntry (FileEntry parent, File f) {
		this.parent = parent;
		this.metadata = new FileMetadata(f);
	}
		...
		if(this.size() != file.listFiles().length) {
			...
			for(File f : file.listFiles()) {
				...
				if(extensionsSet != null) {
					...
					validExtension = true;
				if(validExtension) {
					final FileEntry entry = new FileEntry(this, f);
					this.add(entry);
					wasUpdated = true;
				}
			}
